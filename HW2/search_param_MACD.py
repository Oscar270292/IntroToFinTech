...
def myStrategy(pastPriceVec, currentPrice, short_n, long_n, signal_n):
    import numpy as np
    global short_ema_prev, long_ema_prev, signal_line_prev, macd_prev  # Use global variables to store previous values

    # short_n = 1  # Short-term period for EMA
    # long_n = 19  # Long-term period for EMA
    # signal_n = 9  # Period for the signal line (9-day EMA of MACD)
    action = 0
...
def computeReturnRate(priceVec, short_n, long_n, signal_n):
    ...
    for ic in range(dataCount):
        ...
        suggestedAction[ic] = myStrategy(priceVec[0:ic], currentPrice, short_n, long_n, signal_n)  # Obtain the suggested action
