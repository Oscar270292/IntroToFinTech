...
def construct_mat(flow, route, tot_len):
    ...
    if day == tot_len - 1 and point % 2 == 0:
        mat[0] = day
        mat[1] = point // 2
        mat[2] = -1
...
start_days = [0] * 50
for test_day in range(total_len-K-50+1, total_len-K+1):
    start_days[test_day - (total_len-K-50+1)] = test_day
...
for start_day in start_days:
    ...
    column_991 = np.array(column_991)
    for i in range(0, 8, 2):
        column_991[i] *= priceMat[start_day-1][i // 2] * (1 - transFeeRate)
    ...
    for i in range(start_day+1, start_day+K+1): #為了多一行來初始化
